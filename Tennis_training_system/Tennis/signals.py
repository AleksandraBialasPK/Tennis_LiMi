...
def set_lat_lon(sender, instance, **kwargs):
    ...
    # Check if the court already exists (updating) and if any address-related fields have changed
    if instance.pk:
        old_instance = Court.objects.get(pk=instance.pk)
        address_fields = ['building_number', 'street', 'city', 'postal_code', 'country']
        # If address has changed, update latitude and longitude
    ...
    # If it's a new instance or lat/lon is not set, attempt to get geolocation
...
#     """
#     # Fetch the previous state of the court
#     if instance.pk:
#         try:
#             old_instance = Court.objects.get(pk=instance.pk)
#         except Court.DoesNotExist:
#             # If the court doesn't exist yet (new creation), skip
#             old_instance = None
#
#         if old_instance:
...
#                 # Latitude/Longitude has changed, trigger travel time updates
